...
import { ChevronDown, ChevronRight, Copy, Wrench, BarChart3, Clock, CheckCircle, AlertCircle, Loader2, Brain, Code, Eye, Zap, Image, ChevronUp } from 'lucide-react';
      ...
      {isExpanded && (
        <div className="px-3 pb-3 border-t border-gray-200">
          ...
          if (isCompleted || isPending) {
            return (
              <li key={index} className={`flex items-start gap-3 p-2 rounded-md transition-all ${
                isCompleted 
           ...
           if (line.trim().startsWith('-')) {
            return (
              <li key={index} className="ml-10 text-sm text-gray-500 list-disc">{line.trim().substring(1).trim()}</li>
            )
          }
          if (line.match(/^\d+\./)) {
            return (
          ...
          return (
            <li key={index} className="text-sm text-gray-800">{line}</li>
          );
            })}
          </ul>
        </div>
    ...
    <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-4 rounded-lg border border-blue-200 shadow-sm">
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-blue-700">Analyzing data patterns...</span>
        </div>
    ...
    <div className="bg-gradient-to-r from-green-50 to-emerald-50 p-4 rounded-lg border border-green-200 shadow-lg">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-green-700">Analysis finished successfully</p>
        </div>
...
const TaskSummarySection = ({ content }: { content: string }) => {
  ...
  const completedTasks = lines.filter(line => line.includes('[✓]') || line.includes('[✔]')).length;
  const failedTasks = lines.filter(line => line.includes('[✗]') || line.includes('[×]')).length;
  ...
  return (
    <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl border border-green-200 shadow-lg overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          <CheckCircle className="w-6 h-6 text-green-600" />
          <h3 className="font-bold text-green-800 text-lg">Task Completion Summary</h3>
          <div className="bg-green-100 px-3 py-1 rounded-full">
            <span className="text-sm font-semibold text-green-700">{successRate}% Complete</span>
          </div>
        </div>
        <div className="flex items-center gap-3">
          ...
          <div className="mt-4">
            ...
            <div className="space-y-2">
              {lines.map((line, index) => {
                ...
                if (line.match(/^\d+\./)) {
                  return (
                    <div key={index} className="flex items-start gap-3 p-2 text-gray-700">
...
const StructuredLogViewer = ({ content }: { content: string }) => {
  const parts = [];
  const regex = /(?:## Plan|<plan>)([\s\S]*?)(?=<execute>|<observation>|<solution>|<\/plan>|$)|<execute>([\s\S]*?)<\/execute>|<observation>([\s\S]*?)<\/observation>|<solution>([\s\S]*?)<\/solution>/gi;
  let lastIndex = 0;
  ...
  if (matches.length > 0) {
    for (const match of matches) {
      ...
      if (matchIndex > lastIndex) {
        parts.push(<p key={`preamble-${lastIndex}`} className="text-sm text-gray-700 whitespace-pre-wrap">{cleanedContent.substring(lastIndex, matchIndex)}</p>);
      }

      if (plan) {
        parts.push(<PlanSection key={`plan-${matchIndex}`} content={plan.trim()} />);
      } else if (execute) {
        parts.push(<CodeSection key={`execute-${matchIndex}`} content={execute.trim()} />);
      } else if (observation) {
        parts.push(<ObservationSection key={`observation-${matchIndex}`} content={observation.trim()} />);
      }
    ...
    if (lastIndex < cleanedContent.length) {
      parts.push(<p key={`postamble-${lastIndex}`} className="text-sm text-gray-700 whitespace-pre-wrap">{cleanedContent.substring(lastIndex)}</p>);
    }
  }
  
  if (parts.length === 0 && cleanedContent) {
      // Check if this is a task summary with multiple numbered items and completion status
      const isTaskSummary = cleanedContent.match(/^\d+\.\s*\[[✓✔✗×]\]/m) && 
      ...
      const isResearchQuery = cleanedContent.includes('Compare') || 
                             cleanedContent.includes('analyze') || 
                             cleanedContent.includes('predictive capacity') ||
                             cleanedContent.includes('I need to') ||
                             cleanedContent.match(/^[A-Z][^.]*?(analysis|compare|predictive|capacity|ratio)/i);
      ...
      return <div className="text-sm text-gray-800 font-mono whitespace-pre-wrap">{cleanedContent}</div>
  }
...
export function ExecutorPanel({ className = '', streamingLogs = [], isStreaming = false }: ExecutorPanelProps) {
  ...
  useEffect(() => {
    const allContent = [...streamingLogs, ...executorLogs.map(log => log.content)].join('\n');
  ...
  const parseAndRenderStructuredLog = (content: string) => {
    ...
    const isTaskSummary = filteredContent.match(/^\d+\.\s*\[[✓✔✗×]\]/m) && 
                         filteredContent.split('\n').filter(line => line.match(/^\d+\.\s*\[[✓✔✗×]\]/)).length >= 3;
    
    if (isTaskSummary && !filteredContent.includes('<plan>') && !filteredContent.includes('<execute>')) {
      return <TaskSummarySection content={filteredContent} />;
    }
    
    // Check if this is a research query
    const isResearchQuery = filteredContent.includes('Compare') || 
                           filteredContent.includes('analyze') || 
                           filteredContent.includes('predictive capacity') ||
                           filteredContent.includes('I need to') ||
                           filteredContent.match(/^[A-Z][^.]*?(analysis|compare|predictive|capacity|ratio)/i);
    
    if (isResearchQuery && !filteredContent.includes('<plan>') && !filteredContent.includes('<execute>')) {
    ...
    const planMatch = filteredContent.match(/(?:## Plan|<plan>)([\s\S]*?)(?:<execute>|<observation>|<\/plan>|$)/i);
    const executeMatch = filteredContent.match(/<execute>([\s\S]*?)<\/execute>/i);
    const observationMatch = filteredContent.match(/<observation>([\s\S]*?)<\/observation>/i);
    ...
    if (planMatch || executeMatch || observationMatch) {
      ...
      return (
        <div className="space-y-3">
          {sections.preamble && <p className="text-sm text-gray-700 whitespace-pre-wrap">{sections.preamble}</p>}
          {sections.plan && <PlanSection content={sections.plan} />}
          {sections.execute && <CodeSection content={sections.execute} />}
          {sections.observation && <ObservationSection content={sections.observation} />}
        </div>
      );
    }
      ...
      return (
        <div className="space-y-2">
          {log.content.split('\n').map((line, index) => {
            if (line.includes('[✔]')) {
              return (
                <div key={index} className="flex items-center gap-3 p-2 rounded-md bg-green-50 border border-green-200 text-green-800 shadow-sm">
              ...
              return (
                <div key={index} className="flex items-center gap-3 p-2 rounded-md text-gray-600 hover:bg-gray-50 transition-all">
  ...
  return (
    <div className={`bg-white text-gray-900 h-full flex flex-col border-l border-gray-200 ${className}`}>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
            ...
            {isStreaming && (
              <div className="bg-blue-50 p-3 rounded-lg border border-blue-200 shadow-sm">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-blue-700 font-medium">Processing continues...</span>
                </div>
              </div>
            )}
          </>
        ...
        {isStreaming && (!streamingLogs || streamingLogs.length === 0) && !processingComplete && (
          <StreamingLoader />
        )}
        
        {/* Show completion indicator when processing is done */}
        {processingComplete && !isStreaming && (
          <ProcessingCompleteIndicator />
        )}
        
        {executorLogs.length === 0 && (!streamingLogs || streamingLogs.length === 0) && !isStreaming && !processingComplete ? (
      ...
      <div className="p-4 border-t border-gray-200 bg-gray-50">
        <div className="flex items-center justify-between text-sm">
          <span className={`${processingComplete ? 'text-green-600 font-medium' : 'text-gray-600'}`}>
            {processingComplete ? 'Analysis Complete ✓' : (isProcessing || isStreaming ? 'Processing...' : 'Ready')}
          </span>
          <span className="text-gray-500">
            {(() => {
              ...
              if (streamingLogs && streamingLogs.length > 0) {
                ...
                const taskSummaryMatches = content.match(/^\d+\.\s*\[[✓✔✗×]\]/gm);
                const researchMatches = content.match(/Compare|analyze|predictive capacity|I need to/gi);
                
                streamingSteps = (planMatches?.length || 0) + 
                               (executeMatches?.length || 0) + 
                               (observationMatches?.length || 0) +
                               ((taskSummaryMatches?.length || 0) > 3 ? 1 : 0) +
                               ((researchMatches?.length || 0) > 0 && !planMatches && !executeMatches ? 1 : 0);
              ...
              return totalSteps === 0 ? 'No logs yet' : `${totalSteps} execution step${totalSteps === 1 ? '' : 's'}`;
            })()}
          </span>
        </div>
      </div>
